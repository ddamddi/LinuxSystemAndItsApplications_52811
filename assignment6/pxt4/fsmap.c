...
/* Convert an pxt4_fsmap to an fsmap. */
...
/* Convert an fsmap to an pxt4_fsmap. */
...
static int pxt4_getfsmap_dev_compare(const void *p1, const void *p2)
{
	const struct pxt4_getfsmap_dev *d1 = p1;
	const struct pxt4_getfsmap_dev *d2 = p2;

	return d1->gfd_dev - d2->gfd_dev;
}

/* Compare a record against our starting point */
static bool pxt4_getfsmap_rec_before_low_key(struct pxt4_getfsmap_info *info,
 ...
 */
static int pxt4_getfsmap_helper(struct super_block *sb,
				struct pxt4_getfsmap_info *info,
				struct pxt4_fsmap *rec)
{
...
static int pxt4_getfsmap_datadev_helper(struct super_block *sb,
					pxt4_group_t agno, pxt4_grpblk_t start,
					pxt4_grpblk_t len, void *priv)
{
	...
	list_for_each_entry_safe(p, tmp, &info->gfi_meta_list, fmr_list) {
		if (p->fmr_physical + p->fmr_length <= info->gfi_next_fsblk) {
			...
			error = pxt4_getfsmap_helper(sb, info, p);
			if (error)
				return error;
				...
				struct pxt4_getfsmap_info *info)
{
	...
	trace_pxt4_fsmap_low_key(sb, info->gfi_dev, 0,
			info->gfi_low.fmr_physical,
			info->gfi_low.fmr_length,
			info->gfi_low.fmr_owner);

	trace_pxt4_fsmap_high_key(sb, info->gfi_dev, 0,
...
/* Helper to fill out an pxt4_fsmap. */
static inline int pxt4_getfsmap_fill(struct list_head *meta_list,
				     pxt4_fsblk_t fsb, pxt4_fsblk_t len,
				     uint64_t owner)
{
	struct pxt4_fsmap *fsm;
 ...
 */
static unsigned int pxt4_getfsmap_find_sb(struct super_block *sb,
					  pxt4_group_t agno,
...
static int pxt4_getfsmap_compare(void *priv,
				 struct list_head *a,
				 struct list_head *b)
{
	struct pxt4_fsmap *fa;
	struct pxt4_fsmap *fb;

	fa = container_of(a, struct pxt4_fsmap, fmr_list);
	fb = container_of(b, struct pxt4_fsmap, fmr_list);
...
static void pxt4_getfsmap_free_fixed_metadata(struct list_head *meta_list)
{
	struct pxt4_fsmap *p;
	struct pxt4_fsmap *tmp;
...
static int pxt4_getfsmap_find_fixed_metadata(struct super_block *sb,
					     struct list_head *meta_list)
{
	...
	for (agno = 0; agno < PXT4_SB(sb)->s_groups_count; agno++) {
		...
		/* Superblock & GDT */
		error = pxt4_getfsmap_find_sb(sb, agno, meta_list);
		if (error)
			goto err;
	...
	return 0;
err:
	pxt4_getfsmap_free_fixed_metadata(meta_list);
	return error;
}

/* Execute a getfsmap query against the buddy bitmaps */
static int pxt4_getfsmap_datadev(struct super_block *sb,
				 struct pxt4_fsmap *keys,
				 struct pxt4_getfsmap_info *info)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	pxt4_fsblk_t bofs;
	pxt4_fsblk_t eofs;
	pxt4_group_t start_ag;
	pxt4_group_t end_ag;
	pxt4_grpblk_t first_cluster;
	pxt4_grpblk_t last_cluster;
	int error = 0;

	bofs = le32_to_cpu(sbi->s_es->s_first_data_block);
	eofs = pxt4_blocks_count(sbi->s_es);
	...
	error = pxt4_getfsmap_find_fixed_metadata(sb, &info->gfi_meta_list);
	if (error)
		goto err;
	...
	for (info->gfi_agno = start_ag;
		...
		trace_pxt4_fsmap_low_key(sb, info->gfi_dev, info->gfi_agno,
				info->gfi_low.fmr_physical,
				info->gfi_low.fmr_length,
				info->gfi_low.fmr_owner);

		trace_pxt4_fsmap_high_key(sb, info->gfi_dev, info->gfi_agno,
				info->gfi_high.fmr_physical,
				info->gfi_high.fmr_length,
				info->gfi_high.fmr_owner);
	...
	info->gfi_last = true;
	error = pxt4_getfsmap_datadev_helper(sb, end_ag, last_cluster, 0, info);
	if (error)
		goto err;

err:
	pxt4_getfsmap_free_fixed_metadata(&info->gfi_meta_list);
	return error;
}

/* Do we recognize the device? */
static bool pxt4_getfsmap_is_valid_device(struct super_block *sb,
					  struct pxt4_fsmap *fm)
{
...
static bool pxt4_getfsmap_check_keys(struct pxt4_fsmap *low_key,
				     struct pxt4_fsmap *high_key)
{
		  ...
		  pxt4_fsmap_format_t formatter, void *arg)
{
	...
	if (head->fmh_iflags & ~FMH_IF_VALID)
		return -EINVAL;
	if (!pxt4_getfsmap_is_valid_device(sb, &head->fmh_keys[0]) ||
	    !pxt4_getfsmap_is_valid_device(sb, &head->fmh_keys[1]))
		return -EINVAL;
	...
	memset(&dkeys[1], 0xFF, sizeof(struct pxt4_fsmap));
	...
	for (i = 0; i < PXT4_GETFSMAP_DEVS; i++) {
		...
		if (handlers[i].gfd_dev > head->fmh_keys[0].fmr_device)
			memset(&dkeys[0], 0, sizeof(struct pxt4_fsmap));
