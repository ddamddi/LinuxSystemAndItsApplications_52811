...
/*
 * Extent status tree implementation for pxt4.
 *
 *
 * ==========================================================================
...
void pxt4_es_init_tree(struct pxt4_es_tree *tree)
{
	tree->root = RB_ROOT;
	tree->cache_es = NULL;
}
...
#ifdef ES_DEBUG__
static void pxt4_es_print_tree(struct inode *inode)
{
	...
	if (tree->cache_es) {
		...
		if (in_range(lblk, es1->es_lblk, es1->es_len)) {
			es_debug("%u cached by [%u/%u) %llu %x\n",
				 ...
				 pxt4_es_pblock(es1), pxt4_es_status(es1));
			goto out;
		}
	}
...
static void pxt4_es_free_extent(struct inode *inode, struct extent_status *es)
{
	...
	kmem_cache_free(pxt4_es_cachep, es);
}
				 ...
				 struct extent_status *es2)
{
	...
	if ((pxt4_es_is_written(es1) || pxt4_es_is_unwritten(es1)) &&
	    (pxt4_es_pblock(es1) + es1->es_len == pxt4_es_pblock(es2)))
		return 1;
	...
	if (ex) {
		...
		if (es->es_lblk < ee_block ||
			...
			pr_warn("ES insert assertion failed for inode: %lu "
				...
				pxt4_es_pblock(es), es_status ? 'u' : 'w');
			goto out;
		}
					    ...
					    struct extent_status *es)
{
	...
	/*
	 * Here we call pxt4_ind_map_blocks to lookup a block mapping because
	...
	if (retval > 0) {
		if (pxt4_es_is_delayed(es) || pxt4_es_is_hole(es)) {
			...
			if (map.m_pblk != pxt4_es_pblock(es)) {
				pr_warn("ES insert assertion failed for "
					"inode: %lu m_pblk %llu != "
					"es_pblk %llu\n",
	...
	while (*p) {
		...
		if (newes->es_lblk < es->es_lblk) {
			if (pxt4_es_can_be_merged(newes, es)) {
				...
				if (pxt4_es_is_written(es) ||
				    pxt4_es_is_unwritten(es))
					pxt4_es_store_pblock(es,
							     newes->es_pblk);
				es = pxt4_es_try_to_merge_left(inode, es);
				goto out;
			}
			p = &(*p)->rb_left;
		} else if (newes->es_lblk > pxt4_es_end(es)) {
			if (pxt4_es_can_be_merged(es, newes)) {
				es->es_len += newes->es_len;
				es = pxt4_es_try_to_merge_right(inode, es);
				goto out;
			}
			p = &(*p)->rb_right;
		} else {
...
retry:
	...
	len2 = pxt4_es_end(es) > end ? pxt4_es_end(es) - end : 0;
	if (len1 > 0)
		es->es_len = len1;
	if (len2 > 0) {
		if (len1 > 0) {
			...
			if (pxt4_es_is_written(&orig_es) ||
			    pxt4_es_is_unwritten(&orig_es))
				block = pxt4_es_pblock(&orig_es) +
					orig_es.es_len - len2;
	...
	if (es && es->es_lblk < end + 1) {
		pxt4_lblk_t orig_len = es->es_len;

		len1 = pxt4_es_end(es) - end;
		if (count_reserved)
			  ...
			  pxt4_lblk_t len)
{
	...
	/*
	 * pxt4_clear_inode() depends on us taking i_es_lock unconditionally
...
retry:
	...
	while (nr_to_walk-- > 0) {
		...
		ei = list_first_entry(&sbi->s_es_list, struct pxt4_inode_info,
				      i_es_list);
...
out:
	...
	trace_pxt4_es_shrink(sbi->s_sb, nr_shrunk, scan_time,
			     nr_skipped, retried);
	return nr_shrunk;
}

static unsigned long pxt4_es_count(struct shrinker *shrink,
				   struct shrink_control *sc)
{
	unsigned long nr;
	struct pxt4_sb_info *sbi;

	sbi = container_of(shrink, struct pxt4_sb_info, s_es_shrinker);
	nr = percpu_counter_read_positive(&sbi->s_es_stats.es_stats_shk_cnt);
	trace_pxt4_es_shrink_count(sbi->s_sb, sc->nr_to_scan, nr);
	return nr;
}

static unsigned long pxt4_es_scan(struct shrinker *shrink,
				  struct shrink_control *sc)
{
	struct pxt4_sb_info *sbi = container_of(shrink,
					struct pxt4_sb_info, s_es_shrinker);
	int nr_to_scan = sc->nr_to_scan;
	int ret, nr_shrunk;

	ret = percpu_counter_read_positive(&sbi->s_es_stats.es_stats_shk_cnt);
	trace_pxt4_es_shrink_scan_enter(sbi->s_sb, nr_to_scan, ret);
...
int pxt4_es_register_shrinker(struct pxt4_sb_info *sbi)
{
	int err;
	...
	sbi->s_es_shrinker.scan_objects = pxt4_es_scan;
	sbi->s_es_shrinker.count_objects = pxt4_es_count;
...
void pxt4_es_unregister_shrinker(struct pxt4_sb_info *sbi)
{
	percpu_counter_destroy(&sbi->s_es_stats.es_stats_cache_hits);
	percpu_counter_destroy(&sbi->s_es_stats.es_stats_cache_misses);
	percpu_counter_destroy(&sbi->s_es_stats.es_stats_all_cnt);
...
static int es_reclaim_extents(struct pxt4_inode_info *ei, int *nr_to_scan)
{
	struct inode *inode = &ei->vfs_inode;
	int nr_shrunk = 0;
	pxt4_lblk_t start = ei->i_es_shrink_lblk;
...
#ifdef ES_DEBUG__
static void pxt4_print_pending_tree(struct inode *inode)
{
...
#else
#define pxt4_print_pending_tree(inode)
#endif

int __init pxt4_init_pending(void)
{
	pxt4_pending_cachep = kmem_cache_create("pxt4_pending_reservation",
					   sizeof(struct pending_reservation),
					   0, (SLAB_RECLAIM_ACCOUNT), NULL);
	if (pxt4_pending_cachep == NULL)
		return -ENOMEM;
	return 0;
}
...
void pxt4_init_pending_tree(struct pxt4_pending_tree *tree)
{
	tree->root = RB_ROOT;
}
...
static int __insert_pending(struct inode *inode, pxt4_lblk_t lblk)
{
	...
	pr = kmem_cache_alloc(pxt4_pending_cachep, GFP_ATOMIC);
	if (pr == NULL) {
		ret = -ENOMEM;
		goto out;
	}
...
/*
 * pxt4_remove_pending - removes a pending cluster reservation from the set
 *                       of pending reservations
 *
 * @inode - file containing the cluster
 * @lblk - logical block in the pending cluster reservation to be removed
...
/*
 * pxt4_is_pending - determine whether a cluster has a pending reservation
 *                   on it
 *
 * @inode - file containing the cluster
				     ...
				     pxt4_lblk_t end)
{
	...
	/* guaranteed to be unequal to any pxt4_lblk_t value */
...
/*
 * pxt4_es_delayed_clu - count number of clusters containing blocks that
