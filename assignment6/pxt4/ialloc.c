...
static int pxt4_validate_inode_bitmap(struct super_block *sb,
				      ...
				      struct buffer_head *bh)
{
	...
	if (!pxt4_inode_bitmap_csum_verify(sb, block_group, desc, bh,
...
pxt4_read_inode_bitmap(struct super_block *sb, pxt4_group_t block_group)
{
	...
	if (!buffer_uptodate(bh)) {
		put_bh(bh);
		pxt4_error(sb, "Cannot read inode bitmap - "
			   "block_group = %u, inode_bitmap = %llu",
			   block_group, bitmap_blk);
		pxt4_mark_group_bitmap_corrupted(sb, block_group,
				PXT4_GROUP_INFO_IBITMAP_CORRUPT);
...
void pxt4_free_inode(handle_t *handle, struct inode *inode)
{
	...
	if (unlikely(PXT4_MB_GRP_IBITMAP_CORRUPT(grp))) {
		fatal = -EFSCORRUPTED;
		goto error_return;
	}

	BUFFER_TRACE(bitmap_bh, "get_write_access");
	fatal = pxt4_journal_get_write_access(handle, bitmap_bh);
	if (fatal)
		goto error_return;
	...
	if (gdp) {
		...
		fatal = pxt4_journal_get_write_access(handle, bh2);
	}
	pxt4_lock_group(sb, block_group);
	...
	pxt4_inode_bitmap_csum_set(sb, block_group, gdp, bitmap_bh,
				   PXT4_INODES_PER_GROUP(sb) / 8);
...
static int recently_deleted(struct super_block *sb, pxt4_group_t group, int ino)
{
	struct pxt4_group_desc	*gdp;
 ...
 */
struct inode *__pxt4_new_inode(handle_t *handle, struct inode *dir,
...
repeat_in_this_group:
		...
		if (!handle) {
			BUG_ON(nblocks <= 0);
			handle = __pxt4_journal_start_sb(dir->i_sb, line_no,
							 handle_type, nblocks,
							 0);
			if (IS_ERR(handle)) {
				err = PTR_ERR(handle);
		...
		err = pxt4_journal_get_write_access(handle, inode_bitmap_bh);
		if (err) {
			pxt4_std_error(sb, err);
			goto out;
		}
...
got:
	...
	err = pxt4_journal_get_write_access(handle, group_desc_bh);
	if (err) {
		pxt4_std_error(sb, err);
		goto out;
	}
	...
	if (pxt4_has_group_desc_csum(sb) &&
		...
		err = pxt4_journal_get_write_access(handle, block_bitmap_bh);
		if (err) {
		...
		if (pxt4_has_group_desc_csum(sb) &&
			...
			pxt4_block_bitmap_csum_set(sb, group, gdp,
						   block_bitmap_bh);
	...
	if (pxt4_has_group_desc_csum(sb)) {
		pxt4_inode_bitmap_csum_set(sb, group, gdp, inode_bitmap_bh,
					   PXT4_INODES_PER_GROUP(sb) / 8);
		pxt4_group_desc_csum_set(sb, group, gdp);
	}
	pxt4_unlock_group(sb, group);
...
struct inode *pxt4_orphan_get(struct super_block *sb, unsigned long ino)
{
	...
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		pxt4_error(sb, "couldn't read orphan inode %lu (err %d)",
			   ino, err);
		return inode;
	}
				 ...
				 int barrier)
{
	 ...
	 */
	if (!(gdp->bg_flags & cpu_to_le16(PXT4_BG_INODE_UNINIT)))
		used_blks = DIV_ROUND_UP((PXT4_INODES_PER_GROUP(sb) -
			    pxt4_itable_unused_count(sb, gdp)),
			    sbi->s_inodes_per_block);

	if ((used_blks < 0) || (used_blks > sbi->s_itb_per_group) ||
	    ((group == 0) && ((PXT4_INODES_PER_GROUP(sb) -
			       pxt4_itable_unused_count(sb, gdp)) <
			      PXT4_FIRST_INO(sb)))) {
	...
	BUFFER_TRACE(group_desc_bh, "get_write_access");
	ret = pxt4_journal_get_write_access(handle,
					    group_desc_bh);
	if (ret)
		goto err_out;
