...
/**
 *	pxt4_block_to_path - parse the block number into array of offsets
 ...
 *
 *	To store the locations of file's data pxt4 uses a data structure common
...
static int pxt4_block_to_path(struct inode *inode,
			      pxt4_lblk_t i_block,
			      pxt4_lblk_t offsets[4], int *boundary)
{
	...
	if (i_block < direct_blocks) {
		...
		pxt4_warning(inode->i_sb, "block %lu > max in inode %lu",
...
/**
 *	pxt4_get_branch - read the chain of indirect blocks leading to data
	...
	*err = 0;
	...
	while (--depth) {
		...
		if (!bh_uptodate_or_lock(bh)) {
			...
			if (pxt4_check_indirect_blockref(inode, bh)) {
				put_bh(bh);
				goto failure;
			}
		}
...
/**
 *	pxt4_find_near - find a place for allocation with sufficient locality
 *	@inode: owner
...
/**
 *	pxt4_find_goal - find a preferred place for allocation.
 *	@inode: owner
 *	@block:  block we want
...
/**
 *	pxt4_blks_to_allocate - Look up the block map and count the number
 ...
 */
static int pxt4_blks_to_allocate(Indirect *branch, int k, unsigned int blks,
...
/**
 * pxt4_alloc_branch() - allocate and set up a chain of blocks
 ...
 *	pxt4_alloc_block() (normally -ENOSPC). Otherwise we set the chain
 *	as described above and return 0.
 */
static int pxt4_alloc_branch(handle_t *handle,
			     struct pxt4_allocation_request *ar,
			     int indirect_blks, pxt4_lblk_t *offsets,
			     Indirect *branch)
{
	struct buffer_head *		bh;
	pxt4_fsblk_t			b, new_blocks[4];
	__le32				*p;
	int				i, j, err, len = 1;

	for (i = 0; i <= indirect_blks; i++) {
		if (i == indirect_blks) {
			new_blocks[i] = pxt4_mb_new_blocks(handle, ar, &err);
		} else
		...
		err = pxt4_handle_dirty_metadata(handle, ar->inode, bh);
		if (err)
			goto failed;
	}
	return 0;
failed:
	for (; i >= 0; i--) {
		...
		pxt4_free_blocks(handle, ar->inode, NULL, new_blocks[i],
...
/**
 * pxt4_splice_branch() - splice the allocated branch onto inode.
 * @handle: handle for this transaction
 * @ar: structure describing the allocation request
 ...
 */
static int pxt4_splice_branch(handle_t *handle,
			      struct pxt4_allocation_request *ar,
			      Indirect *where, int num)
{
	int i;
	int err = 0;
	pxt4_fsblk_t current_block;
	...
	if (where->bh) {
		...
		err = pxt4_journal_get_write_access(handle, where->bh);
		if (err)
			goto err_out;
	}
	/* That's it */
	...
	*where->p = where->key;
	...
	if (where->bh) {
		 ...
		 * generic_commit_write->__mark_inode_dirty->pxt4_dirty_inode.
		 */
		jbd_debug(5, "splicing indirect only\n");
		BUFFER_TRACE(where->bh, "call pxt4_handle_dirty_metadata");
		err = pxt4_handle_dirty_metadata(handle, ar->inode, where->bh);
		if (err)
			goto err_out;
	} else {
		/*
		 ...
		 */
		pxt4_mark_inode_dirty(handle, ar->inode);
			...
			int flags)
{
	...
	partial = pxt4_get_branch(inode, depth, offsets, chain, &err);
	...
	*/
	...
	/*
	 * Block out pxt4_truncate while we alter the tree
	 */
	err = pxt4_alloc_branch(handle, &ar, indirect_blks,
				offsets + (partial - chain), partial);

	/*
	 * The pxt4_splice_branch call will free and forget any buffers
	 ...
	 */
	if (!err)
		err = pxt4_splice_branch(handle, &ar, partial, indirect_blks);
	if (err)
		goto cleanup;
 ...
 * contiguous blocks
 */
int pxt4_ind_trans_blocks(struct inode *inode, int nrblocks)
{
	/*
...
/**
 *	pxt4_find_shared - find the indirect blocks for partial truncation.
 *	@inode:	  inode in question
 *	@depth:	  depth of the affected branch
 *	@offsets: offsets of pointers in that branch (see pxt4_block_to_path)
 *	@chain:	  place to store the pointers to partial indirect blocks
 *	@top:	  place to the (detached) top of branch
 *
 *	This is a helper function used by pxt4_truncate().
 *
 ...
 *	past the truncation point is possible until pxt4_truncate()
...
static Indirect *pxt4_find_shared(struct inode *inode, int depth,
				  pxt4_lblk_t offsets[4], Indirect chain[4],
				  __le32 *top)
{
	Indirect *partial, *p;
	int k, err;
	...
	*top = 0;
	...
	if (p == chain + k - 1 && p > chain) {
		...
		*top = *p->p;
		/* Nope, don't do this in pxt4.  Must leave the tree intact */
#if 0
		*p->p = 0;
#endif
	}
 ...
 */
static int pxt4_clear_blocks(handle_t *handle, struct inode *inode,
			     struct buffer_head *bh,
			     pxt4_fsblk_t block_to_free,
			     ...
			     __le32 *last)
{
	...
	if (try_to_extend_transaction(handle, inode)) {
		...
		if (unlikely(err))
			goto out_err;
		err = pxt4_truncate_restart_trans(handle, inode,
	...
	pxt4_free_blocks(handle, inode, NULL, block_to_free, count, flags);
	return 0;
out_err:
...
/**
 * pxt4_free_data - free a list of data blocks
 * @handle:	handle for this transaction
 ...
 * block pointers.
 */
static void pxt4_free_data(handle_t *handle, struct inode *inode,
			   struct buffer_head *this_bh,
			   __le32 *first, __le32 *last)
{
	pxt4_fsblk_t block_to_free = 0;    /* Starting block # of a run */
	unsigned long count = 0;	    /* Number of blocks in the run */
	__le32 *block_to_free_p = NULL;	    /* Pointer into inode/ind
					       corresponding to
					       block_to_free */
	pxt4_fsblk_t nr;		    /* Current block # */
	...
	if (this_bh) {				/* For indirect block */
		...
		err = pxt4_journal_get_write_access(handle, this_bh);
	...
	for (p = first; p < last; p++) {
		...
		if (nr) {
			...
			if (count == 0) {
				...
				err = pxt4_clear_blocks(handle, inode, this_bh,
						        block_to_free, count,
						        block_to_free_p, p);
				if (err)
					break;
	...
	if (!err && count > 0)
		err = pxt4_clear_blocks(handle, inode, this_bh, block_to_free,
	...
	if (this_bh) {
		BUFFER_TRACE(this_bh, "call pxt4_handle_dirty_metadata");
...
/**
 *	pxt4_free_branches - free an array of branches
 ...
 *	appropriately.
 */
static void pxt4_free_branches(handle_t *handle, struct inode *inode,
			       ...
			       __le32 *first, __le32 *last, int depth)
{
	...
	if (depth--) {
		...
		while (--p >= first) {
			...
			pxt4_free_branches(handle, inode, bh,
					(__le32 *) bh->b_data,
			...
			if (parent_bh) {
				...
				if (!pxt4_journal_get_write_access(handle,
								   parent_bh)){
					*p = 0;
					BUFFER_TRACE(parent_bh,
					"call pxt4_handle_dirty_metadata");
					pxt4_handle_dirty_metadata(handle,
								   inode,
								   parent_bh);
				}
			}
		...
		pxt4_free_data(handle, inode, parent_bh, first, last);
	}
}
...
void pxt4_ind_truncate(handle_t *handle, struct inode *inode)
{
	...
	__le32 nr = 0;
	int n = 0;
	pxt4_lblk_t last_block, max_block;
	 ...
	 * pxt4 *really* writes onto the disk inode.
	 */
	...
	partial = pxt4_find_shared(inode, n, offsets, chain, &nr);
	/* Kill the top of shared branch (not detached) */
	if (nr) {
		if (partial == chain) {
			/* Shared branch grows from the inode */
			pxt4_free_branches(handle, inode, NULL,
			...
			*partial->p = 0;
			...
			pxt4_free_branches(handle, inode, partial->bh,
					partial->p,
	...
	while (partial > chain) {
		pxt4_free_branches(handle, inode, partial->bh, partial->p + 1,
...
/**
 *	pxt4_ind_remove_space - remove space from the range
 *	@handle: JBD handle for this transaction
 *	@inode:	inode we are dealing with
			  ...
			  pxt4_lblk_t start, pxt4_lblk_t end)
{
	...
	n = pxt4_block_to_path(inode, start, offsets, NULL);
	n2 = pxt4_block_to_path(inode, end, offsets2, NULL);
	...
	if ((n == 1) && (n == n2)) {
		...
		partial = p = pxt4_find_shared(inode, n, offsets, chain, &nr);
		if (nr) {
			if (partial == chain) {
				/* Shared branch grows from the inode */
				pxt4_free_branches(handle, inode, NULL,
...
end_range:
		partial2 = p2 = pxt4_find_shared(inode, n2, offsets2, chain2, &nr2);
		if (nr2) {
			if (partial2 == chain2) {
				/*
		...
		while (partial2 > chain2) {
			pxt4_free_branches(handle, inode, partial2->bh,
